...
mod create_donor {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("nourishnet_3", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let donation_amount: u64 = Default::default();
		let fee: u8 = Default::default();

		let delegate_info = Keypair::new();
		let (donor_pubkey, _) = Pubkey::find_program_address(
			&[b"donor"],
			&program_id,
		);
		let accounts = vec![
			...
			AccountMeta::new(donor_pubkey, false),
			AccountMeta::new_readonly(system_program::id(), false),
		];
		let data = Nourishnet3Instruction::CreateDonor(crate::generated::instructions::CreateDonorArgs{
			donation_amount,
			fee,
		});
		let signers = vec![
